...
<script>
import { GetUserinfo, uploadPicturesBase64, changeNick,UserAccount } from "@/api";
...
export default {
  ...
  methods: {
    ...
    async Submit() {
      ...
      if (!this.$store.state.user.is_anchor && !this.$store.state.user.info.is_live) {
        this.$message({
            message: "先要完成实名认证",
            type: "warning",
        });
    ...
    getImage(image) {
      ...
      console.log(44444, this.imageUrl)
    },
    ...
    updatephoto() {
      setTimeout(() => {
        ...
        console.log(data, 0);
        if (data.file !== "") {
          this.bus.$emit("ishoveimg",this.imageUrl)
  ...
  .content_val {
    ...
    .coin {
      ...
      .coinlogo {
        ...
        width: 20px;
        height: 24px;
        background: url('../assets/images/longzuan.png');
        position: relative;
        top: 6px;
      }
