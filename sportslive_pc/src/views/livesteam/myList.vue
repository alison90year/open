      ...
      <div class="top-t" v-if="anchorlist.length > 0">
        <div v-for="(item, index) in anchorlist13" :key="index" class="top1">
          ...
          <div
            v-if="item.is_live === 1"
            class="zhibozhong"
            @click="zbckick(item, index)"
          >
            <img src="../../assets/img/zhibozhong.gif" alt="" />
          </div>
          <div v-else>
            <div
            ...
            <!-- <div v-else class="noguangzhu">已关注</div> -->
          </div>
        </div>
        <div></div>
      </div>
        ...
        <ul>
          <li>
            ...
            <div v-else>
              ...
              <div v-if="item.is_follow === 1">已关注</div>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    getanchorlist(type) {
      ...
      anchorlist(params).then((res) => {
        ...
        if (res.code === 0) {
          ...
          this.anchorlist = res.info.slice(3, 10);
          let data = {
            live_user: {
              avatar: "http://107.148.224.65:84/upload/default.png",
              nick_name: "虚位以待",
            },
            //  for(i=0;i<(10-this.anchorlist.length);i++){

            //  }
          };
          console.log(7 - this.anchorlist.length);
          let k = this.anchorlist.length;
          let j = this.anchorlist13.length;
          for (var i = 0; i < 3 - j; i++) {
            console.log("111111111111111111111111111111111111111111111111");
            this.anchorlist13.push(data);
          }
          for (var i = 0; i < 7 - k; i++) {
            console.log("111111111111111111111111111111111111111111111111");
    ...
    getgoldmaster(type) {
      ...
      goldmaster(params).then((res) => {
        ...
        if (res.code === 0) {
          ...
          this.goldlist = res.info.slice(3, 10);
          let data = {
            live_user: {
              avatar: "http://107.148.224.65:84/upload/default.png",
              nick_name: "虚位以待",
          ...
          console.log(3 - this.goldlist.length);
          let k = this.goldlist.length;
          let j = this.goldlist13.length;
          for (var i = 0; i < 3 - j; i++) {
            this.goldlist13.push(data);
          }
          for (var i = 0; i < 7 - k; i++) {
            this.goldlist.push(data);
          }
    ...
    zbckick(item, index) {
      this.$router.push({
        name: "liveRoom",
        query: { liveuid: item.uid, stream: item.stream },
      });
    },
    qxckick(item, iindex) {
      cancelFollowAnchor;
      console.log(item, iindex);
      this.getcancelFollowAnchor(item.live_uid);
    },
    getcancelFollowAnchor(live_uid) {
      const params = {
        uid: this.user.id,
        token: this.token,
        live_uid: live_uid,
      };
      cancelFollowAnchor(params).then((res) => {
        if (res.code === 0) {
          this.$message({
            type: "success",
            message: res.msg,
...
.renqi {
  width: 430px;
  min-height: 800px;
 background: linear-gradient(to bottom, #fffaed 1%, #fff 99%);
