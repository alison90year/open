<template>
  <div class="focus">
    <ul v-if="lives.length>0">
      <li v-for="(item, index) in lives" :key="index">
        <div class="usericon">
          <img :src="item.avatar" alt="" />
        </div>
        <div class="gzcontent">
          <p>
            {{ item.nick_name }} <i>{{ item.level }}</i>
          </p>
          <span>突然发现，能够不变，有时候就是一种最难得的幸福。</span>
        </div>
        <div class="isgz">
          <span @click="getcancelFollowAnchor(item)">取消关注</span>
        </div>
      </li>
    </ul>
...
import { userFollowAnchor, FollowAnchor, cancelFollowAnchor } from "@/api";
export default {
  components:{
    noData
  },
