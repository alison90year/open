    ...
    <el-dialog title="注册" :visible.sync="showRegister" width="400px" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-form :model="registerForm" :rules="registerRule" ref="registerForm">
        <div style="display: flex;justify-content: center;">
          <el-select v-model="registerForm.value" placeholder="+86" style="width: 25%;">
            <el-option v-for="item in Areacode" :key="item.value" :value="item.value">{{item.label}}{{item.value}}</el-option>
          </el-select>
          <el-form-item prop="mobile" style="width: 75%;">
            <el-input v-model="registerForm.mobile" placeholder="请输入手机号码"></el-input>
          </el-form-item>
        </div>

        <el-form-item prop="code">
          <el-input v-model="registerForm.code" placeholder="请输入龙牙直播验证码">
            <template slot="append">
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.registerForm.checked) {
            ...
            this.anchorcode = this.registerForm.code
            window.localStorage.setItem("anchorcode",  this.anchorcode)
