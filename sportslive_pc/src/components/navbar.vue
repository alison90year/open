  ...
  >
      ...
      <div class="header-right" v-if="isLogin">
        <el-popover width="400" trigger="hover">
          ...
          <el-row class="grzxlist">
            ...
            <el-col :span="6">
              <img src="@/assets/img/zhubo.png" alt @click="Submit(4)" />
              <p @click="Submit(4)">我要直播</p>
            </el-col>
            <!--                        <el-col :span="6">-->
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitfk()" style="background: linear-gradient(90deg,#eccbab, #dbb16f 100%);border:none;">提交反馈</el-button>
      </span>
    </el-dialog>
    <div
      id="asker"
    ...
    >
      ...
      <el-tooltip class="item" effect="light" placement="left">
        <div slot="content" style="color: #dbb16f">
      ...
      <br />
      <el-tooltip class="item" effect="light" placement="left">
        <div slot="content" style="color: #dbb16f; text-align: center">
          <img width="60px" src="../assets/images/gongzonghao.png" /><br />公众号
        </div>
...
import liveNoble from "../views/livesteam/liveNoble";
import Liveylist from "../views/livesteam/liveylist";
import fullMoney from "../views/fullMoney";
import mytuijian from "../views/livesteam/mytuijian";
export default {
  name: "navbar",
  components: { fullMoney },
  inject: ["reload"],
  props: {
    active: {
      type: Boolean,
  ...
  mounted() {
    ...
    this.init();
    let that = this;
    this.bus.$on("ishome", (message) => {
      //这里最好用箭头函数，不然this指向有问题
      that.ishome = message;
      // let active = location.hash
      console.log(active);
      that.navList.forEach((item, index) => {
        if (active == "#/LiveType/fixturelist" || active == "#/mine/myHome") {
          that.navList[2].active = true;
          that.navList[0].active = false;
        }
      });
    });
  },
  methods: {
    ...
    changeMask(type) {
      if (type == 1) {
        this.$refs.longya.style.display = "block";
      } else {
        this.$refs.longya.style.display = "none";
      }
    },
    init() {
